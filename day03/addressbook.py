...
def del_address(lst_contact, name):
    result = False # 아무것도 삭제 안했음음
    for i, item in enumerate(lst_contact): # 각 개체에 번호를 매겨주는 클래스
        if item.isNameExist(name):
            del lst_contact[i] # del은 메모리에서 완전삭제
            result = True # 지웠다

    return result #삭제여부 반환환

# 8. 콘솔화면 클리어 함수
def clear_console():
    command = 'clear' # LINUX, UNIX 클리어 명령어
    if os.name in ('nt', 'dos'): # 운영체계가 윈도우라면
        command = 'cls' # Windows 클리어 명령령어

    os.system(command) # 콘솔에 명령어 실행행
...
def load_contact(lst_contact):
    ...
    while True:
        line = f.readline() # 예찬/7732/naver.com/부산
        if not line:
...
if __name__ == '__main__':
    # pass는 #if, for, while 함수 등에서 오류를 없애는 방법
    # print('주소록 프로그램')
    # 3. 클래스 실행 - 객체를 생성
    # first = contact.Contact('이예찬', '010-4853-5172', 'yechan5172@daum.net', '부산')
    # print(first)
    run() # 제일 처음 실행되는 함수
